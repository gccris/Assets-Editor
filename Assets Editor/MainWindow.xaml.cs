...
namespace Assets_Editor
{
    ...
    {
        public static  string _assetsPath = "C:/Users/gabri/OneDrive/Documentos/Tibia Related/clientes/Client 14/assets";

        public static  string _assetsPathOriginal = "C:/Users/gabri/OneDrive/Documentos/Tibia Related/clientes/Client 14 Clean/tibia-client-13.16.13209/assets";
        public static string _datPath = "";
        public static string _datPathOriginal = "";
        ...
        public static List<Catalog> catalogOriginal;

        public static Appearances appearances;      
        public static Appearances appearancesOriginal;  
        ...
        private void LoadCatalogJsonOriginal()
        {
            using StreamReader r = new StreamReader(_assetsPathOriginal + "catalog-content.json");
            string json = r.ReadToEnd();
            var settings = new JsonSerializerSettings
            {
                NullValueHandling = NullValueHandling.Ignore,
            ...
            catalogOriginal = JsonConvert.DeserializeObject<List<Catalog>>(json, settings);
        }
        ...
        private void LoadAppearancesOriginal()
        {
            _datPathOriginal = String.Format("{0}{1}", _assetsPathOriginal, catalogOriginal[0].File);
            if (File.Exists(_datPathOriginal) == false)
                return;
            FileStream appStream;
            using (appStream = new FileStream(_datPathOriginal, FileMode.Open, FileAccess.ReadWrite, FileShare.Read))
            {
                appearancesOriginal = Appearances.Parser.ParseFrom(appStream);
                ObjectCount = (ushort)appearancesOriginal.Object[^1].Id;
                OutfitCount = (ushort)appearancesOriginal.Outfit[^1].Id;
                EffectCount = (ushort)appearancesOriginal.Effect[^1].Id;
                MissileCount = (ushort)appearancesOriginal.Missile[^1].Id;

                ObjectsCount1.Content = ObjectCount;
                OutfitsCount1.Content = OutfitCount;
                EffectsCount1.Content = EffectCount;
                MissilesCount1.Content = MissileCount;
            }
        }
        ...
        private void SelectAssetsFolderOriginal(object sender, RoutedEventArgs e)
        {
            FolderBrowserDialog _assets = new FolderBrowserDialog();
            if (_assets.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                _assetsPathOriginal = _assets.SelectedPath;
                if (_assetsPathOriginal.EndsWith("\\") == false)
                    _assetsPathOriginal += "\\";
                AssetsPath_Original.Text = _assetsPathOriginal;
            }
            if (_assetsPathOriginal != "" && File.Exists(_assetsPathOriginal + "catalog-content.json") == true)
            {

                LoadCatalogJsonOriginal();
                LoadAppearancesOriginal();
                LoadAssets.IsEnabled = true;
            }
            else
                MessageBox.Show("You have selected a wrong assets path for original.");
        }
        ...
        private void LoadSprSheet()
        {
            ...
            /*if (catalogOriginal.Count > 0){
                Parallel.ForEach(catalogOriginal, options, (spr, state) =>
                {
                    if (spr.Type == "sprite")
                    {
                        string _sprPath = String.Format("{0}{1}", _assetsPathOriginal, spr.File);
                    ...
                    progress++;
                    worker.ReportProgress((int)(progress * 100 / catalogOriginal.Count));
                });
                for (uint i = 0; i < catalogOriginal.Max(r => r.LastSpriteid) + 1; i++)
                {
